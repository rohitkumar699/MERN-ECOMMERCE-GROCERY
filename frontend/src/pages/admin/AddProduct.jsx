        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          <div>
            ...
            >
              ...
              {categoriesData
                .filter((cat) => typeof cat._id === "string" && cat._id.length === 24)
                .map((category) => (
          ...
          <div className="grid grid-cols-2 gap-4">
            {[0, 1, 2, 3].map((index) => (
              <div key={index}>
                ...
                >
                  {previews[index] ? (
                    <img src={previews[index]} className="h-full object-contain" alt="preview" />
                  ) : (
                    <>
                      <Upload className="w-6 h-6 text-gray-400 mb-1" />
                      <span className="text-gray-500 text-xs">Upload image {index + 1}</span>
                    </>
                  )}
                </label>
                {formData.images[index] && (
                  <p className="text-xs text-gray-500 mt-1 text-center">{formData.images[index].name}</p>
                )}
              </div>
            ))}
          </div>
